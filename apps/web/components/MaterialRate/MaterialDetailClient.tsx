...
function getAllImages(rate: MaterialDetailClientProps['rate']): string[] {
  const all: string[] = [];
...
export default function MaterialDetailClient({ rate, materialType }: MaterialDetailClientProps) {
  ...
  const detailBase = materialSlug(materialType);
  const unitLabel  = rate.unit || 'unit';
    ...
    <div className="pt-24 bg-white min-h-screen">
      <div className="max-w-6xl mx-auto px-3 py-5">
        ...
        <nav className="flex items-center gap-1 text-xs text-gray-400 mb-4">
          ...
          <span>/</span>
          <Link href={`/${detailBase}`} className="hover:text-black transition-colors">
            Today {materialType} Rate in Pakistan
          </Link>
          <span>/</span>
        ...
        <div className="grid lg:grid-cols-2 gap-5">
          ...
          <div className="space-y-4">
            ...
            <div className="border border-gray-200 rounded-lg px-4 py-3 flex items-center justify-between">
              ...
              <p className="text-xs text-gray-400">per {unitLabel}</p>
            </div>
            ...
            <div className="grid grid-cols-2 gap-2">
              {[
                ...
                { icon: Tag,     label: 'Unit',          value: rate.unit },
                { icon: Layers,  label: 'Category',      value: rate.category ?? rate.materialType },
                { icon: Package, label: 'Material Type', value: rate.materialType },
            ...
            <div className="flex items-center gap-3">
              ...
              {qty >= MAX_QTY && (
                <span className="text-[10px] text-red-500 font-medium">Max 10</span>
              )}
            </div>
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <a
                href={`https://wa.me/${CONTACT_NUMBER}?text=${encodeURIComponent(`Hi, I'm interested in ${rate.brand} (${qty} ${qty > 1 ? 'units' : 'unit'}) — Rs ${rate.price}/${unitLabel}`)}`}
                target="_blank"
        ...
        <div id="section-additional" className="mt-4 border border-gray-200 rounded-lg overflow-hidden scroll-mt-20">
          ...
          <div className="p-5">
            <table className="w-full text-sm">
              <tbody>
                {[
                  { label: 'Brand',         value: rate.brand },
                  { label: 'Price',          value: `Rs ${rate.price.toLocaleString()} / ${unitLabel}` },
                  { label: 'City',           value: rate.city },
                  { label: 'Unit',           value: rate.unit },
                  { label: 'Category',       value: rate.category ?? rate.materialType },
                  { label: 'Material Type',  value: rate.materialType },
