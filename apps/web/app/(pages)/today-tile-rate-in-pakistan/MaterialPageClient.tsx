...
export default function MaterialPageClient({
  ...
  const showTileNav = materialType === "Tile" && tileCategories.length > 0;
        ...
        <div className="bg-white border-b border-gray-200 pt-20">
          ...
          <div className="max-w-6xl mx-auto px-4 py-4">
            <div className="flex gap-4 overflow-x-auto scrollbar-hide pb-2">
              ...
              >
                <div className={`w-28 h-20 rounded-lg overflow-hidden border-2 transition-all ${
                  activeCatSlug === "all" ? "border-black" : "border-transparent"
                }`}>
 <img
  src="/alltiles.jpg"
  alt="All Tiles"
...
/>                </div>
                <span className={`text-xs font-medium whitespace-nowrap ${
                  activeCatSlug === "all" ? "text-black" : "text-gray-600 group-hover:text-black"
                }`}>
                  All Tiles
                </span>
              </button>
              ...
              {tileCategories.map((cat) => (
                <button
                  ...
                  onMouseEnter={() => handleCatMouseEnter(cat.slug)}
                  onMouseLeave={handleCatMouseLeave}
                  className="flex flex-col items-center gap-2 shrink-0 group relative"
                >
                  <div className={`w-28 h-20 rounded-lg overflow-hidden border-2 transition-all ${
                    activeCatSlug === cat.slug ? "border-black" : "border-transparent group-hover:border-gray-300"
                  }`}>
                    {cat.image ? (
                      <img
                        src={cat.image}
                  ...
                  <span className={`text-xs font-medium whitespace-nowrap ${
                    activeCatSlug === cat.slug ? "text-black" : "text-gray-600 group-hover:text-black"
                  }`}>
                    {cat.name}
                  </span>
                </button>
          ...
          {/* Subcategory dropdown — shows on hover */}
          {hoveredCat && (() => {
            const cat = tileCategories.find((c) => c.slug === hoveredCat);
            if (!cat || cat.subcategories.length === 0) return null;
            return (
              <div
                className="border-t border-gray-100 bg-white shadow-md"
                onMouseEnter={() => handleCatMouseEnter(hoveredCat)}
                onMouseLeave={handleCatMouseLeave}
              >
                <div className="max-w-6xl mx-auto px-4 py-4">
                  <div className="flex flex-wrap gap-x-8 gap-y-2">
                    <span className="text-xs font-bold text-gray-900 w-full mb-1">
                      {cat.name}
                    </span>
                    {cat.subcategories.map((sub) => (
                      <a
                        key={sub.slug}
                        href={`/today-tile-rate-in-pakistan/${sub.slug}`}
          ...
          <div className="border-t border-gray-100 overflow-x-auto scrollbar-hide">
            <div className="max-w-6xl mx-auto px-4 flex gap-0">
              <button
                onClick={() => setActiveCatSlug("all")}
                className={`text-sm px-4 py-3 border-b-2 whitespace-nowrap transition-colors ${
                  activeCatSlug === "all"
      ...
      {/* ── PAGE BODY ────────────────────────────────────────────────── */}
      <div className={`${!showTileNav ? "pt-24" : ""}`}>
          ...
          <div className="flex gap-6 items-start">
            ...
            >
              ...
              <FilterSection title="Brands">
                {BRANDS.map((b) => (
                  <RadioOption key={b} name="brand" label={b} checked={selectedBrand === b}
                    onChange={() => { setSelectedBrand(b); setCurrentPage(1); }}
              ...
              <FilterSection title={`${materialType} Type`}>
                {CATEGORIES.map((cat) => (
                  <RadioOption key={cat} name="category" label={cat} checked={selectedCat === cat}
                    onChange={() => { setSelectedCat(cat); setCurrentPage(1); }} />
                ))}
              </FilterSection>
            </aside>
            ...
            <main className="flex-1 min-w-0">
              ...
              {!pageContent && (
                <div className="mt-12 pt-8 border-t border-gray-200">
                  <h2 className="text-lg font-bold text-gray-900 mb-3">{pageTitle} — 2026</h2>
