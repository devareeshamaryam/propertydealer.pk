...
export default function EditSandRatePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await api.post('/sand-rate', fd, {
        headers: { 'Content-Type': 'multipart/form-data' },
      });
      
      toast.success('Sand rate added successfully!');
      router.push('/dashboard/sand-rate');
    } catch (err: any) {
      toast.error(err?.response?.data?.message || 'Failed to create sand rate');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div className="w-full max-w-3xl space-y-6">
      ...
      <div className="bg-white rounded-xl shadow-sm border p-8">
        <h1 className="text-2xl font-bold text-gray-900 mb-1">Edit Sand Rate</h1>
        <p className="text-gray-500 text-sm mb-8">This will appear on the public Sand Rate page.</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex items-center gap-3 pt-2">
            <Button type="submit" disabled={submitting}>
              {submitting ? <><Loader2 className="w-4 h-4 animate-spin mr-2" /> Saving…</> : 'Save Sand Rate'}
            </Button>
            <Button type="button" variant="outline" onClick={() => router.push('/dashboard/sand-rate')}>
              Cancel
            </Button>
          </div>
        </form>
