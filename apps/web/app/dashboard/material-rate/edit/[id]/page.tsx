...
export default function EditMaterialRatePage() {
  ...
  useEffect(() => {
    if (!id) return;
    materialRateApi.getRateById(id)
      .then(rate => {
        setForm({
          materialType: rate.materialType
            ? rate.materialType.charAt(0).toUpperCase() + rate.materialType.slice(1)
            : 'Door',
          brand:       rate.brand ?? '',
  ...
  return (
    <div className="w-full max-w-3xl space-y-6">
      ...
      <div className="bg-white rounded-xl shadow-sm border p-8">
        <h1 className="text-2xl font-bold text-gray-900 mb-1">Edit Material Rate</h1>
        <p className="text-gray-500 text-sm mb-8">Update the price or details for this material brand.</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex items-center gap-3 pt-2">
            <Button type="submit" disabled={submitting} className="flex items-center gap-2">
              {submitting ? <><Loader2 className="w-4 h-4 animate-spin" /> Saving…</> : 'Update Material Rate'}
            </Button>
            <Button type="button" variant="outline" onClick={() => router.push('/dashboard/material-rate')}>Cancel</Button>
          </div>
        </form>
      </div>
    </div>
