...
export default function EditCityPage() {
  ...
  useEffect(() => {
    const fetchCity = async () => {
      try {
        ...
        toast.error("Error", {
          description: error?.response?.data?.message || "Failed to load city.",
        });
  ...
  const removeTypeContent = (i: number) =>
    form.setValue(
      "typeContents",
      (form.getValues("typeContents") || []).filter((_, idx) => idx !== i),
    );

  const addSizeContent = () => {
    const cur = form.getValues("sizeContents") || [];
    form.setValue("sizeContents", [
      ...cur,
      {
        size: "5marla",
        purpose: "all",
  ...
  const removeSizeContent = (i: number) =>
    form.setValue(
      "sizeContents",
      (form.getValues("sizeContents") || []).filter((_, idx) => idx !== i),
    );
  ...
  const updateAreaSizeContentField = (
    idx: number,
    field: keyof SizeContentItem,
    value: string,
  ) => {
    setAreaSizeContents((prev) =>
              ...
              <div className="grid grid-cols-1 gap-6 p-4 bg-gray-50 rounded-lg border border-gray-100">
                ...
                <div className="mt-8 space-y-6">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-sm text-gray-500">
                        Add SEO content for specific marla / kanal sizes, e.g.
                        "5 Marla House for Sale in{" "}
                        {form.watch("name") || "City"}"
                      </p>
                    </div>
                    <Button
                      type="button"
                  ...
                  {form.watch("sizeContents")?.map((_, index) => (
                    <div
                      key={index}
                      className="p-6 bg-white border border-orange-100 rounded-xl shadow-sm space-y-4 relative group"
                    >
                      <Button
                        type="button"
                        variant="ghost"
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <FormField
                          control={form.control}
                          name={`sizeContents.${index}.purpose`}
                          render={({ field }) => (
                            <FormItem>
                              <FormLabel>Purpose</FormLabel>
                              <Select
                      ...
                      <p className="text-xs text-gray-400 font-mono bg-gray-50 px-3 py-2 rounded">
                        /properties/
                        {form.watch(`sizeContents.${index}.purpose`) || "all"}/
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <FormField
                          control={form.control}
                          name={`sizeContents.${index}.metaTitle`}
                          render={({ field }) => (
                            <FormItem>
                              <FormLabel>Meta Title</FormLabel>
                              <FormControl>
                        ...
                        <FormField
                          control={form.control}
                          name={`sizeContents.${index}.metaDescription`}
                          render={({ field }) => (
                            <FormItem>
                              <FormLabel>Meta Description</FormLabel>
                              <FormControl>
                      ...
                      <FormField
                        control={form.control}
                        name={`sizeContents.${index}.content`}
                        render={({ field }) => (
                          <FormItem>
                            <FormLabel>Rich Content</FormLabel>
                            <FormControl>
                  ...
                  {(!form.watch("sizeContents") ||
                    form.watch("sizeContents")?.length === 0) && (
                    <div className="text-center py-8 bg-gray-50 rounded-xl border border-dashed border-orange-200">
                      <p className="text-gray-500 italic">
                        No size-specific content added yet.
                      </p>
                      <p className="text-xs text-gray-400 mt-1">
                        Click "Add Size Content" to add 2/3/5/10 Marla or 1
                        Kanal SEO content.
                      </p>
                    </div>
                  )}
                ...
                <div className="mt-8 space-y-6 border-t pt-8">
                  <div>
                    ...
                    <p className="text-sm text-gray-500">
                      Select an existing area or add a new one, then add
                      size-wise SEO content for it. e.g. "10 Marla House for
                      Sale in DHA Phase 7, {form.watch("name") || "City"}"
                    </p>
                  </div>
                    ...
                    <div className="space-y-4">
                      <div className="flex items-center justify-between">
                        <h4 className="text-md font-semibold text-gray-700">
                          Size Specific Content for this Area
                        </h4>
                        <Button
                          type="button"
                          variant="outline"
                          size="sm"
                          onClick={addAreaSizeContent}
                          className="flex items-center gap-2"
                        >
                          <PlusCircle className="h-4 w-4" />
                          Add Size Content
                      ...
                      {areaSizeContents.map((item, index) => (
                        <div
                          key={index}
                          className="p-4 bg-gray-50 border border-gray-200 rounded-xl space-y-4 relative group"
                        >
                          <Button
                            type="button"
                            variant="ghost"
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                              ...
                              <Select
                                value={item.size}
                                onValueChange={(val) =>
                                  updateAreaSizeContentField(index, "size", val)
                                }
                              >
                                <SelectTrigger>
                                  <SelectValue placeholder="Select size" />
                      ...
                      {areaSizeContents.length === 0 && (
                        <div className="text-center py-6 bg-gray-50 rounded-xl border border-dashed border-gray-300">
                          <p className="text-gray-500 italic text-sm">
                            No size-specific content added for this area yet.
                          </p>
                        </div>
                      )}
                    </div>
