...
export default function EditAreaPage() {
    ...
    areaApi.getById(areaId)
      .then((area) => {
        ...
        form.reset({
          ...
          rentContent: area.rentContent || "",
          saleMetaTitle: area.saleMetaTitle || "",
          saleMetaDescription: area.saleMetaDescription || "",
          saleContent: area.saleContent || "",
        });
      })
      .catch((error: any) => {
        toast.error("Error", { description: error?.response?.data?.message || "Failed to load area." });
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    console.log("Submitting Area Values:", values);
    try {
      const payload = {
      ...
      await areaApi.update(areaId, payload);
      toast.success("Area updated successfully!");
      // Adding a slight delay before redirecting to allow backend to finish save and avoid stale cache
      setTimeout(() => {
        router.push("/dashboard/area");
      }, 500);
    } catch (error: any) {
      console.error("Update Error:", error);
      toast.error("Error", { description: error?.response?.data?.message || error?.message || "Failed to update area." });
    }
  }
    ...
    <div className="w-full">
      <div className="max-w-5xl mx-auto">
        <div className="bg-white rounded-xl shadow-lg p-8">
          ...
          <p className="text-gray-600 mb-8">Update area information for property listings.</p>
              ...
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                ...
                <div className="p-4 bg-blue-50 rounded-lg border border-blue-100 space-y-4">
                  <div>
                    ...
                    <p className="text-sm text-blue-500">Shown on pages like /properties/rent/city/area. If left empty, general SEO is used.</p>
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="p-4 bg-green-50 rounded-lg border border-green-100 space-y-4">
                  <div>
                    ...
                    <p className="text-sm text-green-500">Shown on pages like /properties/sale/city/area. If left empty, general SEO is used.</p>
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="flex gap-4 pt-6">
                  <Button type="submit" disabled={isLoading || loadingCities} className="flex-1 bg-gray-800 hover:bg-gray-900">
                    {isLoading ? <><Loader2 className="mr-2 h-5 w-5 animate-spin" />Updating...</> : "Update Area"}
                  </Button>
