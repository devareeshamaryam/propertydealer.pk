...
export default function EditSteelRatePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await api.post('/steel-rate', fd, {
        headers: { 'Content-Type': 'multipart/form-data' },
      });
      
      toast.success('Steel rate added successfully!');
      router.push('/dashboard/steel-rate');
    } catch (err: any) {
      toast.error(err?.response?.data?.message || 'Failed to create steel rate');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div className="w-full max-w-3xl space-y-6">
      ...
      <div className="bg-white rounded-xl shadow-sm border p-8">
        <h1 className="text-2xl font-bold text-gray-900 mb-1">Edit Steel Rate</h1>
        <p className="text-gray-500 text-sm mb-8">This will appear on the public Steel Rate page.</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex items-center gap-3 pt-2">
            <Button type="submit" disabled={submitting}>
              {submitting ? <><Loader2 className="w-4 h-4 animate-spin mr-2" /> Saving…</> : 'Save Steel Rate'}
            </Button>
            <Button type="button" variant="outline" onClick={() => router.push('/dashboard/steel-rate')}>
              Cancel
            </Button>
          </div>
        </form>
