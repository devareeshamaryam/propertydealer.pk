...
export class WoodRateController {
  ...
  async findAllAdmin() {
    return this.woodRateService.findAllAdmin();
  }

  @Get(':id')
  @UseGuards(JwtAuthGuard, AdminGuard)
  async findOne(@Param('id') id: string) {
    return this.woodRateService.findById(id);
  }
  ...
  async create(@Request() req) {
    try {
      console.log('📝 Creating wood rate...');
      ...
      const result = await this.woodRateService.create(dto);
      console.log('✅ Wood rate created:', result._id);
      return result;
    } catch (error) {
      console.error('❌ Error creating wood rate:', error);
      throw error;
    }
  }
  ...
  async update(@Param('id') id: string, @Request() req) {
    try {
      ...
      return this.woodRateService.update(id, dto);
    } catch (error) {
      console.error('❌ Error updating wood rate:', error);
      throw error;
    }
  }
  ...
  async remove(@Param('id') id: string) {
    return this.woodRateService.remove(id);
  }
}
