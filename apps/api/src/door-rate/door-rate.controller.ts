...
@Controller('door-rate')
export class DoorRateController {
  constructor(
    private readonly doorRateService: DoorRateService,
  ...
  async findAll(
    ...
    if (this.useUnified) {
      return this.materialRates.findAll(city, 'Door', category);
    }
    return this.doorRateService.findAll(city, category);
  }
  ...
  async findBySlug(@Param('slug') slug: string) {
    if (this.useUnified) {
      ...
      return this.materialRates.findBySlug(`door-${slug}`);
    }
    return this.doorRateService.findBySlug(slug);
  }

  @Get('admin/all')
  @UseGuards(JwtAuthGuard, AdminGuard)
  async findAllAdmin() {
    return this.doorRateService.findAllAdmin();
  }

  @Get(':id')
  @UseGuards(JwtAuthGuard, AdminGuard)
  async findOne(@Param('id') id: string) {
    return this.doorRateService.findById(id);
  }
  ...
  async create(@Request() req) {
    try {
      console.log('📝 Creating door rate...');
      ...
      if (imageFile) {
        const key = await this.storageService.upload(imageFile, 'door-rates');
      ...
      for (const file of extraFiles) {
        const key = await this.storageService.upload(file, 'door-rates');
        extraUrls.push(this.storageService.getUrl(key));
      }

      const dto: CreateDoorRateDto = {
        brand:       body.brand,
        price:       Number(body.price),
        change:      body.change ? Number(body.change) : 0,
        city:        body.city,
        unit:        body.unit || 'Per Door',
      ...
      const result = await this.doorRateService.create(dto);
      console.log('✅ Door rate created:', result._id);
      return result;
    } catch (error) {
      console.error('❌ Error creating door rate:', error);
      throw error;
    }
  }
  ...
  async update(@Param('id') id: string, @Request() req) {
    try {
      ...
      return this.doorRateService.update(id, dto);
    } catch (error) {
      console.error('❌ Error updating door rate:', error);
      throw error;
    }
  }
  ...
  async remove(@Param('id') id: string) {
    return this.doorRateService.remove(id);
  }
}
