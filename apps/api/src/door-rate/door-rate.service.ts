...
export class DoorRateService {
  ...
  async findAll(city?: string, category?: string) {
    ...
    return this.doorRateModel.find(query).sort({ createdAt: -1 }).lean().exec();
  }

  async findAllAdmin() {
    return this.doorRateModel.find().sort({ createdAt: -1 }).lean().exec();
  }
  ...
  async findBySlug(slug: string): Promise<DoorRateDocument> {
    let rate = await this.doorRateModel.findOne({ slug }).exec();
    if (!rate && isValidObjectId(slug)) {
      rate = await this.doorRateModel.findById(slug).exec();
    }
